use crate::{
  grpc::insert_api_key,
  http::{AppState, auth::ApiKey, dto::ChainDto, error::ApiHttpError},
};
use axum::{
  Json,
...
pub async fn list_chains(
  ...
  let req = cpay::api::v1::merchant::chain::ListChainsRequest {};
...
pub async fn list_assets(
  State(mut state): State<AppState>,
  Extension(ApiKey(api_key)): Extension<ApiKey>,
  Path(id): Path<super::types::ChainCode>,
) -> Result<impl IntoResponse, ApiHttpError> {
  print!("chain code: {id:?}, {id}");
  let chain_id: cpay::blockchain::v1::Chain = super::types::ChainCode::from(id).into();

  let req = cpay::api::v1::merchant::asset::ListAssetsRequest {
  ...
  let mut assets: Vec<super::dto::AssetDto> = Vec::with_capacity(resp.assets.len());
  for a in resp.assets.into_iter() {
    ...
    assets.push(super::dto::AssetDto {
      id: a.id,
...
pub async fn create_payment_intent(
  State(mut state): State<AppState>,
  Extension(ApiKey(api_key)): Extension<ApiKey>,
  Json(req): Json<super::dto::CreatePaymentIntentRequest>,
) -> Result<impl IntoResponse, ApiHttpError> {
  ...
  let amount_oneof = if let Some(v) = req.amount_usd {
    ...
    Some(cpay::api::v1::merchant::payment::create_intent_request::Amount::AmountUsd(s))
  } else if let Some(v) = req.amount_asset {
    let s = v.to_string_normalized();
    Some(cpay::api::v1::merchant::payment::create_intent_request::Amount::AmountAsset(s))
  } else {
    return Err(ApiHttpError::BadRequest("provide amount_usd or amount_asset".into()));
  };

  let grpc_req = cpay::api::v1::merchant::payment::CreateIntentRequest {
  ...
  Ok(Json(super::dto::PaymentIntentDto {
